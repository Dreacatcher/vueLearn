...
function getServiceDate(url,methods, data, callback) {
	data=dataEncode(data);
	$.ajax({
    	url: url,
...
function general_query(pageNum,position,type){
	...
	if(form_validator(document.forms[0])){
		submitXMLDoc2Div(document.forms[0],app_path+'/common/generalquery_result.do',position);
	}
}
...
{
	var htmlurl = app_path + "/public/dictionary/dictionaryTreeOrList.do?method=treeOrList&XT_TYKJ_KJMC="+kjmc+"&inputKey="+inputKey+"&inputName="+inputName;
...
function initJoinSelect(elements,dictionary,condition,value,upelementvalue){
	...
	if(document.all(elementArr[0])!=null){
		...
		if(dict_list.indexOf("," + dictionary + ",")>=0){
			...
			var url = app_path + "/public/dictionary/dictionarySelect.do?method=getQueryList&XT_TYKJ_KJMC=" + dictionary + "&change=null&CONDITION=" + condition + "&returnList=&returnInputList=&rootName=";
...
function printCertOCX(certno, txcode) {
	var left = (screen.availWidth - 450) / 2;
	var top = (screen.availHeight - 150) / 2;
	url = app_path + "/certprint/card/print/CertPrintService.print.do?CERT_NO=" + certno + "&TRANS_CODE=" + txcode;
	window.open(url, "_blank", "top=" + top + ",left=" + left + ",width=450,height=150,location=no,status=no,resizable=no,menubar=no,titlebar=no,toolbar=no");
}
...
function blockUI(message){
	if(message==null){
		//message = "正在请求，请耐心等候...";
		//message = "<img src="+app_path+"/work/common/images/wait.gif>";
	}
	$.blockUI({
		css:{
			border:           'none',
